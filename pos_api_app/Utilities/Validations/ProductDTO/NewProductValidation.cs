...
public class NewProductValidation : AbstractValidator<NewProductDTO>
{
	...
	public NewProductValidation(IProductRepository productRepository)
	{
		...
		RuleFor(attr => attr.BarcodeID)
		    ...
		    .MustAsync(UniqueBarcode).WithMessage("Product was already registered");

		RuleFor(attr => attr.Title)
		    .NotEmpty().WithMessage("Title is required");
	}

	public async Task<bool> UniqueBarcode(string property, CancellationToken cancellationToken)
	{
		return await _productRepository.UniqueBarcode(property);
	}
}
